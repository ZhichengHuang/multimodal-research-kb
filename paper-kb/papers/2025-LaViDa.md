...
- **互补**:
  ...
  - [[2026-NAP]]: NAP 可利用 LaViDa FIM 能力（dLLM 天然支持并行生成的架构证据）+ Complementary Masking 与 NAP 并行数据训练结合
  - [[2026-Omni-Diffusion]]: Omni-Diffusion 将 dLLM 扩展到三模态（text+image+speech），与 LaViDa 的 dLLM VLM 路线互补
...
- `combines_with` → [[2026-Omni-Diffusion]]: Omni-Diffusion 的 Pre-Infilling 输出格式控制和 LaViDa 的 FIM infilling 共享 MDM 初始序列可编辑的独有特性；Complementary Masking 可迁移到三模态训练场景
